...
class CTRLitDesktop:
    def __init__(self, root):
        self.root = root
        self.root.title("CTRLit Desktop")
        self.root.geometry("500x700")  # Increased height for new elements
        ...
        self.access_code = ""
        self.qr_image = None
        self.web_url = ""  # Store the web URL
        self.direct_url = ""  # Store the direct access URL
    ...
    def setup_ui(self):
        ...
        self.direct_label = ttk.Label(self.qr_frame, text="Direct Access: Not Connected", wraplength=350)
        ...
        # Copy URL buttons
        ttk.Button(self.qr_frame, text="Copy URL to Clipboard", command=self.copy_url_to_clipboard).pack(pady=5)
        ttk.Button(self.qr_frame, text="Copy Direct URL", command=self.copy_direct_url).pack(pady=5)
        
        # Test button for mouse movement
        ttk.Button(self.qr_frame, text="Test Mouse Movement", command=self.test_mouse_movement).pack(pady=5)
    ...
    def connect(self):
        ...
        try:
            # Use the root path with no /socket
    ...
    def disconnect(self):
        ...
        try:
            ...
            self.direct_label.config(text="Direct Access: Not Connected")
    ...
    def copy_url_to_clipboard(self):
        ...
        if hasattr(self, 'web_url') and self.web_url:
            try:
                pyperclip.copy(self.web_url)
                self.log("URL copied to clipboard")
            except Exception as e:
                self.log(f"Error copying to clipboard: {str(e)}")
        else:
            self.log("No URL available. Connect to server first.")
            
    def copy_direct_url(self):
        """Copy the direct access URL to clipboard"""
        if hasattr(self, 'direct_url') and self.direct_url:
            try:
                pyperclip.copy(self.direct_url)
                self.log("Direct access URL copied to clipboard")
            except Exception as e:
                self.log(f"Error copying to clipboard: {str(e)}")
        else:
            self.log("No direct URL available. Connect to server first.")
    
    def test_mouse_movement(self):
        """Test function to verify mouse movement works"""
        self.log("Testing mouse movement...")
        # Move mouse in a small square pattern
        for dx, dy in [(50, 0), (0, 50), (-50, 0), (0, -50)]:
            try:
                pyautogui.move(dx, dy)
                self.log(f"Moved mouse by dx={dx}, dy={dy}")
                time.sleep(0.5)
            except Exception as e:
    ...
    def generate_qr_code(self, code):
        try:
            ...
            try:
                # This gets an IP that's more likely to work across your network
            ...
            self.direct_label.config(text=f"Direct Access: {self.direct_url}")
    ...
    def handle_message(self, message_body):
        try:
            ...
            if msg_type == 'access_code':
                ...
                # Get current mouse position and move it based on touchpad input
                # Apply a larger multiplier to make movement more noticeable
                current_x, current_y = pyautogui.position()
                try:
                    # Use pyautogui.move instead of moveTo for relative movement
                    pyautogui.move(dx, dy)
                    self.log(f"Mouse moved to {current_x + dx}, {current_y + dy}")
                except Exception as e:
                    self.log(f"Mouse movement error: {str(e)}")
